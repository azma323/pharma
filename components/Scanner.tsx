        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="relative">
            ...
            <input
              ref={searchInputRef}
              type="text"
              className={`w-full bg-slate-900 border-2 text-white rounded-4xl py-5 pl-14 pr-6 focus:outline-none transition-all shadow-lg text-lg ${mode === 'sales' ? 'focus:border-amber-400 border-slate-800' : 'focus:border-emerald-500 border-slate-800'}`}
          ...
          <div className={`relative aspect-video bg-slate-900 rounded-[3rem] overflow-hidden border-4 transition-all ${flash ? 'border-white scale-[1.01]' : 'border-slate-800'}`}>
            {isScanning ? <div id="reader" className="w-full h-full"></div> : (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-10">
                ...
                <button onClick={startScanner} className={`px-10 py-4 rounded-4xl font-black uppercase text-xs ${mode === 'sales' ? 'bg-amber-400 text-slate-950 shadow-amber-900/20' : 'bg-emerald-500 text-slate-950 shadow-emerald-900/20'} shadow-2xl transition-transform hover:scale-105`}>Start Camera Scanner</button>
          ...
          <div className={`p-6 rounded-4xl border-2 transition-all flex items-center gap-4 ${status.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : status.type === 'error' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-slate-900/50 border-slate-800 text-slate-500'}`}>
                 ...
                 {history.map((item) => (
                   <div key={item.id} className="bg-slate-800/50 p-4 rounded-2xl flex items-center justify-between border border-slate-700/50 hover:border-white/10 transition-all">
                      <div className="flex items-center gap-4">
                         ...
                         <div><p className="text-sm font-bold text-white truncate max-w-37.5">{item.productName}</p><p className="text-[10px] text-slate-500 font-bold uppercase">SKU: {item.sku}</p></div>
                      </div>
